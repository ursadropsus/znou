...
The most defensible current description is therefore **paired-relation
construction sensitivity**: 541 is sensitive to combinations of two lexical
members and a compatible relational connector, with graded tolerance for
order, carrier, and lexical familiarity. This is a working phenotype, not a
complete feature interpretation. The experiment does not yet separate
opposition, alternation, conventional binomiality, semantic relatedness, and
corpus familiarity cleanly enough to name one of them as the neuron’s feature.
...
canonical result. The connector participates in the route.
...
## 10. Recommended next experiment — PROPOSAL

The next highest-value experiment is a matched relation panel rather than a
larger unsystematic phrase list. It should cross several relation classes with
multiple lexical samples per class:
...
discriminating preliminary phenotype. Neuron 541 is sensitive to the joint
lexical and relational structure of paired constructions. The response is
more abstract than an isolated word or fixed phrase, while remaining graded,
pair-dependent, connector-dependent, and entangled with the Resonance
competitor field.

`CAUTION` — “paired-relation construction sensitivity” is the best current
working label, not a final neuron name. The next useful step is to separate
semantic relation from conventionality and lexical familiarity using multiple
matched held-out samples.
