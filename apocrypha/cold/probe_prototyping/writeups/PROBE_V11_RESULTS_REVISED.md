...
reading of the v11 outputs and a cross-check against v9 minimisation results,
while preserving the first v11 report as a separate artifact. Tags follow
`SPEC.md`: `MEASURED`, `DERIVED`, `INTERPRETATION`, `PROPOSAL`.
...
mean peak of 1.414—1.172 units above paired arbitrary distinct-member forms.
...
The result separates two contributions. Novel connected repetition is
sufficient to raise 541 reproducibly, including with unfamiliar lexical
material, but was insufficient to make 541 the destination in any tested case.
Familiar lexeme–connector combinations amplify that structural response into
the strong, often route-winning regime. Semantic or otherwise related distinct
members occupy a similar intermediate band to novel repetition.

The connector sweep initially appears to reverse the v10 ranking: under exact
repetition, `by`, `after`, and `to` retained 541 more often than `and` or `or`.
An exploratory post-hoc split shows that most of the raw-peak difference comes
from swept forms such as `day by day`, `one to one`, `round by round`, and
`time after time` being familiar constructions themselves. Among forms coded
as novel, all five connector means occupy a narrow 0.35-unit peak band. The
result therefore supports construction-level connector–frame–lexeme
compatibility more strongly than a connector identity ranking.

The pronominal branch supplies the run's strongest evidence for a reusable
slot structure. `One after another`, `one after one`, and `one after the other`
all respond strongly, while `gravel after another` and connectorless `one
another` collapse. This generalises beyond literal duplication and the exact
v9 phrase list. Unknown training-corpus exposure prevents it from excluding a
broader frequency or memorisation account by itself.
...
`INTERPRETATION` — novel exact connected repetition is sufficient to raise
neuron 541 systematically beyond arbitrary distinct coordination. It was
insufficient, under these lexical controls and carriers, to make 541 the
destination in any tested case.
...
The last two examples show why a destination-only account is incomplete:
familiar-looking frames can evoke substantial target activation while losing
to a stronger competitor. `Round and round` has an additional cross-probe
explanation developed below.
...
### 5.4 `Round and round` confirms v9 context dependence — MEASURED,
DERIVED

V11's bare and short-carrier forms of `round and round` retained 541 in 0/4
cases despite a mean target peak of 3.256. V9 had already indicated that the
bare pair was not independently sufficient. Five Melville routes whose minima
ended in `round and round` reduced only to longer certificates:

| V9 token-minimal certificate | Tokens | Signed margin |
|---|---:|---:|
| `hair braided and coiled round and round` | 9 | 0.039820 |
| `and thus round and round` | 5 | 0.203032 |
| `go round and round` | 5 | 0.053954 |
| `water there, wheeling round and round` | 8 | 0.113642 |
| `y Dick swam swiftly round and round` | 8 | 0.000351 |

Not one v9 route reduced to the bare repeated pair, and every minimum had a
thin positive margin. V11's 0/4 result is therefore a successful cross-probe
confirmation of the context dependence observed during v9 minimisation rather
than an anomalous failure of an otherwise sufficient phrase.

`Time and time` occupies a similar low-margin position in v11, but it lacks
this independent v9 minimisation evidence and should not be treated as the same
confirmed case.
...
The aggregate retention ranking differs from v10’s distinct-member connector
matrix, where `or` retained 22/24 and `and` retained 18/24, while `by`, `to`,
and `after` were much weaker. The raw-peak ranking is much less sharply
separated and is substantially confounded by familiar constructions created
during substitution, as the exploratory split in §7.3 shows. V11 confirms that
v10 measured connector performance inside one lexical frame rather than
general connector strength; it does not establish the reverse connector
hierarchy.
...
### 7.3 Exploratory familiarity split — DERIVED, INTERPRETATION
...
To examine this confound, the 60 unique connector/member constructions were
classified post hoc as familiar or novel-looking. The familiar set was:

```text
again/round/time and X
day/mile/one/round/step/zone by X
day/mile/one/step/time/wave after X
face/hand/one/shoulder to X
```

Here `X` repeats the first member. No `or` form was coded familiar. This coding
was performed post hoc after the results were available and was not measured
from a corpus, so the split is exploratory and potentially judgment-sensitive
rather than a preregistered conventionality test.

| Connector | Familiar n | Familiar mean peak | Novel n | Novel mean peak | Novel retention |
|---|---:|---:|---:|---:|---:|
| `and` | 12 | 3.137 | 36 | 2.719 | 0/36 |
| `by` | 24 | 3.844 | 24 | 2.677 | 7/24 |
| `after` | 24 | 3.505 | 24 | 2.402 | 5/24 |
| `or` | 0 | — | 48 | 2.382 | 2/48 |
| `to` | 16 | 3.524 | 32 | 2.373 | 6/32 |

Among forms coded novel, the mean target peaks occupy a narrow 0.35-unit band,
from 2.373 to 2.719, and `and` is highest rather than `by`. The apparent
aggregate peak advantage for `by`, `after`, and `to` is therefore largely
associated with those connectors creating familiar expressions. Retention
still differs among the novel forms, but it is a thresholded outcome built on
similar target peaks and changing competitors; it should not be treated as a
clean connector dissociation.
...
several effective constructions. The post-hoc split suggests constructional
familiarity explains much of the aggregate peak ranking, while leaving open
why similarly activating novel forms cross the destination threshold at
different rates.
...
## 9. Pronominal recurrence provides the strongest slot-structure evidence
...
response, but it depends on compatible lexical and connector material. The
three strong `one after ...` forms preserve a first slot and connector while
allowing `another`, `one`, or `the other` to complete the second slot. This is
the run's clearest evidence for a reusable constructional frame rather than a
detector for duplicated token identity.

`One after one` was not among the exact v9 certificate families and is less
idiomatic than the other two forms, so its 3.900 mean peak is especially
informative. It does not, however, prove corpus novelty. GPT-2 may have seen
that sequence during training or generalized from related memorized strings.
The branch rules out a narrow inventory limited to the exact v9 phrases; it
does not by itself rule out broader frequency- or memory-based accounts.
...
A **repetition-alone route-sufficiency account** is also incomplete. Novel
connected repetition was sufficient to raise 541 reproducibly but never made
it the destination, and familiar repetitions were about 2.14 peak units
stronger on average.
...
both familiar and novel repetition. The repeated-member sweep repaired v10's
frame mismatch, while an exploratory familiarity split showed that its
aggregate connector ranking was driven largely by familiar expressions rather
than connector identity alone. All three `one after ...` completions responded
strongly while lexical and connector controls collapsed, providing the run's
clearest evidence for reusable slot structure. All 114 retained v11 cases
peaked at construction completion.
...
relation, familiarity, and connector–frame compatibility. Novel connected
repetition is sufficient to raise the target reproducibly but was insufficient
to make it the destination in the tested panel.
