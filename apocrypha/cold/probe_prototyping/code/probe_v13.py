"""
probe_v13.py — staged phenotype and causal probe for GPT-2 Small L5-N38.

The recovered “cold” experiments repeatedly added +10 to neuron teams during
greedy generation.  They nominated L5-N38 and companions 1103/2094, but did
not isolate them and did not separate a one-step logit effect from recurrent
autoregressive feedback.  V13 therefore treats those files as exploratory
provenance and runs four explicitly separated branches:

  PHENOTYPE
      Token traces, destinations, direct-logit directions, and contiguous
      source-span certificates for a declared cold/polysemy/control panel.

  NATURAL_ABLATION
      Scale or ablate N38 at the final token of a completed predicate and
      measure a declared continuation.  Includes exact repeat, sham, previous
      position, declared companion, cold-constellation, three matched-neuron,
      and three-neuron-team controls.

  PRE_ADJECTIVE_STEERING
      Add positive and negative N38 doses immediately before a declared
      adjective, and separately test old team members and their combination.
      Doses are derived from the clean natural N38 activation distribution.

  GENERATION_FEEDBACK
      Compare a one-step intervention with the same intervention repeated at
      every greedy generation step.  Natural-range doses are primary; +10 is
      retained only as a labelled legacy replication.

No causal outcome is defined using N38's own activation or Atlas destination.
Teacher-forced outcomes are downstream logits, probabilities, ranks, KL, and
hidden-state changes.  Generation strings are exploratory trajectory records.

Requires the CUDA-corrected znou_probe.py beside this file.

Examples
--------
  python probe_v13.py --smoke
  python probe_v13.py
  python probe_v13.py --outdir results --generation-tokens 15
"""
...
def steering_treatments(doses):
    ...
    return [
        ...
        Treatment("CLEAN_REPEAT", (), "DECLARED", "RERUN", role="deterministic duplicate"),
        Treatment("SHAM_38", (TARGET,), "DECLARED", "SHAM", role="hook identity"),
        Treatment("ADD_POS_LOW_38", (TARGET,), "DECLARED", "ADD", doses["LOW"], "natural-range positive dose"),
        Treatment("ADD_POS_MED_38", (TARGET,), "DECLARED", "ADD", doses["MED"], "natural-range positive dose"),
        Treatment("ADD_POS_HIGH_38", (TARGET,), "DECLARED", "ADD", d, "natural-range positive dose"),
        Treatment("ADD_POS_MAX_38", (TARGET,), "DECLARED", "ADD", doses["MAX"], "maximum observed positive dose"),
        Treatment("ADD_NEG_HIGH_38", (TARGET,), "DECLARED", "ADD", -d, "signed direction control"),
        Treatment("PREVIOUS_ADD_POS_HIGH_38", (TARGET,), "PREVIOUS", "ADD", d, "position control"),
        Treatment("ADD_POS_HIGH_1103", (1103,), "DECLARED", "ADD", d, "companion isolation at shared N38-calibrated dose"),
        Treatment("ADD_POS_HIGH_2094", (2094,), "DECLARED", "ADD", d, "companion isolation at shared N38-calibrated dose"),
        Treatment("ADD_POS_HIGH_TEAM", (38, 1103, 2094), "DECLARED", "ADD", d, "recovered specialist team at shared N38-calibrated dose"),
        Treatment("ADD_POS_HIGH_1508", (1508,), "DECLARED", "ADD", d, "cold-constellation isolation at shared N38-calibrated dose"),
        Treatment("ADD_POS_HIGH_234", (234,), "DECLARED", "ADD", d, "cold-constellation isolation at shared N38-calibrated dose"),
    ]
...
def parse_args():
    ...
    p.add_argument("--smoke", action="store_true",
                   help="small panel; full controls and validation")
    p.add_argument("--generation-tokens", type=int, default=15)
...
def main():
    ...
    device = next(zp.mdl.parameters()).device
    if device.type != "cuda":
        raise SystemExit(f"znou_probe model is on {device}; v13 requires CUDA")
    if not 1 <= args.generation_tokens <= 64:
        raise SystemExit("--generation-tokens must be in 1..64")

    natural = list(NATURAL_CASES[:3] if args.smoke else NATURAL_CASES)
    steering = list(STEERING_CASES[:3] if args.smoke else STEERING_CASES)
    cases = natural + steering
    located = {c.case_id: locate(c) for c in cases}
    print(f"model device: {device}")
    print(f"teacher-forced cases: {len(cases)}")
    ...
    for i, case in enumerate(cases, 1):
        ...
        if i == 1 or i == len(cases):
            print(f"clean [{i}/{len(cases)}]")

    controls, selection = matched_controls(clean_runs, natural, located)
    doses = natural_doses(clean_runs, natural, located)
    print(f"matched controls: {controls}")
    print("N38 natural doses: " + ", ".join(f"{k}={v:.6f}" for k,v in doses.items()))

    treatment_map = {"NATURAL_ABLATION": natural_treatments(controls),
                     "PRE_ADJECTIVE_STEERING": steering_treatments(doses)}
    results = []
    ...
    phenotype_strings = list(PHENOTYPE_STRINGS[:6] if args.smoke else PHENOTYPE_STRINGS)
    print(f"phenotype strings: {len(phenotype_strings)}")
    phenotype, trace, certificates = phenotype_and_certificates(phenotype_strings)
    logit_direction = direct_logit_rows()

    generation, generation_steps = [], []
    if not args.skip_generation:
        prompts = GENERATION_PROMPTS[:2] if args.smoke else GENERATION_PROMPTS
        print(f"generation prompts: {len(prompts)}")
        generation, generation_steps = generation_rows(
            prompts, doses["HIGH"], args.generation_tokens)

    summaries = summary_rows(results)
    effects = token_effect_rows(results)
    factorial = factorial_rows(results)
    preflight = preflight_rows(cases, located, clean_runs, selection, doses)
    failures = validate(cases, results, controls)
    print("\nvalidation")
    print(f"  intervention rows       {len(results)}")
    print(f"  phenotype rows          {len(phenotype)}")
    ...
    outputs = []
    datasets = (
        ("probe_v13_preflight", preflight),
        ("probe_v13_phenotype", phenotype),
        ("probe_v13_trace", trace),
        ("probe_v13_certificates", certificates),
        ("probe_v13_logit_direction", logit_direction),
        ("probe_v13_interventions", results),
        ("probe_v13_token_effects", effects),
        ("probe_v13_factorial", factorial),
        ("probe_v13_summary", summaries),
        ("probe_v13_generation", generation),
        ("probe_v13_generation_steps", generation_steps),
    )
    ...
    for path in outputs:
        print(f"  {path}")
    if failures:
        raise SystemExit("v13 validation failed: " + "; ".join(failures))
