...
def generate_dimensional_report(case_file, probe_data, all_probes, output_dir):
    ...
    with open(report_path, "w") as f:
        ...
        f.write("\n--- I. Ablation Fingerprint Analysis ---\n")
        f.write(f"This section identifies the core neurons for each pole of the axis after ablating landmarks.\n")
        ...
        for concept, data in case_file["axis_concepts"].items():
            ...
            fingerprints[concept] = set(nid for nid, count in most_common) # Store for later analysis
            ...
            neuron_labels = [f"J5-{nid}" for nid, count in most_common]; frequencies = [count for nid, count in most_common]
            plt.style.use('dark_background'); fig, ax = plt.subplots(figsize=(12, 8))
            bars = ax.barh(neuron_labels, frequencies, color=data["color"]); ax.invert_yaxis()
            ax.set_xlabel('Frequency in Top-30'); ax.set_title(f"Ablation Fingerprint for: '{concept.upper()}'")
            ax.grid(axis='x', linestyle='--', alpha=0.3)
            for bar in bars: ax.text(bar.get_width() + 0.1, bar.get_y() + bar.get_height()/2., f'{bar.get_width()}', va='center')
            plt.tight_layout(); plt.savefig(os.path.join(output_dir, f"{case_file['name']}_{concept}_fingerprint.png")); plt.close()

        # --- II. Shared Axis Circuitry (Intersection) ---
        f.write("\n\n--- II. Shared Axis Circuitry ---\n")
        f.write("This section identifies neurons that are core members of BOTH the positive and negative poles, representing the underlying axis itself.\n\n")
        shared_neurons = fingerprints.get('positive', set()) & fingerprints.get('negative', set())
        if shared_neurons:
            f.write("Shared Neurons: " + ", ".join(f"J5-{nid}" for nid in sorted(list(shared_neurons))) + "\n")
        else:
            f.write("No significant shared circuitry found between the top 15 members of each pole.\n")
        ...
        # Save visual heatmap
        plt.figure(figsize=(20, 18)); sns.heatmap(similarity_matrix, xticklabels=all_probes, yticklabels=all_probes, cmap="viridis", annot=False)
        plt.title(f"Activation Vector Cosine Similarity for: '{case_file['name'].upper()}'"); plt.xticks(rotation=90); plt.yticks(rotation=0)
        plt.tight_layout(); plt.savefig(os.path.join(output_dir, f"{case_file['name']}_heatmap.png")); plt.close()
        ...
        f.write("Key Insight: Note the bright intra-concept squares (e.g., cold-cold) and dark inter-concept blocks (e.g., cold-hot, cold-multilingual).\n")


        # --- IV. Detailed Probe-by-Probe Breakdown ---
        f.write("\n\n--- IV. Detailed Probe-by-Probe Ablation Results ---\n")
        # (This section can be very long, so it's generated but we won't print it to console)
        for concept, data in case_file["axis_concepts"].items():
...
if __name__ == "__main__":
    # --- The Case File ---
    CASE_FILE = {
        "name": "AXIS_OF_TEMPERATURE",
        "target_neuron": 38,
        "axis_concepts": {
            "positive": {
                "name": "Cold", "color": "#29eaff",
                "probes": [
                    "It was cold", "cold", "it was freezing", "freezing", "it was icy", "icy",
                    "it was frigid", "frigid", "it was chilly", "chilly", "winter is coming",
                    "the arctic tundra", "a blast of cold air", "absolute zero",
                ]
            },
            "negative": {
                "name": "Hot", "color": "#ff7f29",
                "probes": [
                    "It was hot", "hot", "it was burning", "burning", "it was scorching",
                    "scorching", "a warm day", "the desert sun", "a heatwave",
                ]
            },
            "neutral": {
                "name": "Lukewarm", "color": "#ffff7f",
                "probes": [ "It was lukewarm", "lukewarm", "room temperature" ]
            }
        },
        "landmarks": { 1888: ".", 1790: "?" }
    }

    # Add other probes for similarity analysis, but not for fingerprinting
    all_probes = CASE_FILE["axis_concepts"]["positive"]["probes"] + \
                 CASE_FILE["axis_concepts"]["negative"]["probes"] + \
                 CASE_FILE["axis_concepts"]["neutral"]["probes"]
                 
    # For a clean heatmap, we'll add other control probes here
    control_probes = {
        "structural": ["The day was cold", "I am cold", "She felt cold"],
        "orthogonal": ["a happy dog", "the price of steel"],
        "multilingual": ["Il faisait froid", "hacía frío", "寒い"]
    }
    
    # Temporarily add these to a unified structure for processing, but we'll only use the main axis concepts for fingerprinting.
    full_probe_set = {**CASE_FILE["axis_concepts"], **control_probes}

    analyze_conceptual_axis(CASE_FILE, mode='inference')